/*
	OneLoneCoder.com - Command Line Tetris (macOS Port)
	...
	macOS Port: Converted from Windows Console API to ncurses
*/
...
bool IsKeyPressed(int key)
{
    int ch = getch();
    if (ch != ERR) {
        ungetch(ch); // Put the character back
        return ch == key;
    }
    return false;
}
...
int main()
{
	...
	while (!bGameOver) // Main Loop
	{
		...
		for (int x = 0; x < nFieldWidth; x++)
			for (int y = 0; y < nFieldHeight; y++)
			{
				char fieldChar = " ABCDEFG=#"[pField[y*nFieldWidth + x]];
				mvaddch(y + 2, x + 2, fieldChar);
			}
		...
		for (int px = 0; px < 4; px++)
			for (int py = 0; py < 4; py++)
				if (tetromino[nCurrentPiece][Rotate(px, py, nCurrentRotation)] != L'.')
					mvaddch(nCurrentY + py + 2, nCurrentX + px + 2, nCurrentPiece + 65);

		// Draw Score
		mvprintw(2, nFieldWidth + 6, "SCORE: %8d", nScore);
